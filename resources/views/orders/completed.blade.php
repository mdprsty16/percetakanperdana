<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Riwayat Pesanan Saya (Selesai)') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <h3 class="text-2xl font-bold mb-6 text-center md:text-left">Daftar Riwayat Pesanan Anda</h3>

                    @if ($orders->isEmpty())
                        <p class="text-center text-gray-500 dark:text-gray-400">Anda belum memiliki pesanan yang
                            selesai.</p>
                    @else
                        {{-- Gunakan struktur tabel untuk tampilan riwayat yang ringkas --}}
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        {{-- Kolom Pelanggan tidak ada di sini karena ini adalah riwayat user itu sendiri --}}
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                            Total
                                        </th>
                                    ...
                                    @foreach ($orders as $order)
                                        ...
                                        <tr class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200"
                                            onclick="openOrderDetailModal({{ json_encode([
                                                'id' => $order->id,
                                                'customerName' => $order->user->name, // Tetap ambil nama user meski tidak ditampilkan di tabel, untuk modal
    ...
    {{-- MODAL UNTUK DETAIL RIWAYAT PESANAN PENGGUNA --}}
    <div id="orderHistoryDetailModal"
        ...
        <div
            ...
            {{-- Untuk user, tidak ada tombol "Edit Pesanan Ini" --}}
        </div>
    </div>
